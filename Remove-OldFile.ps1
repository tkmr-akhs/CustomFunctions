...
.DESCRIPTION
指定されたディレクトリから、特定の条件（日付、空き容量など）に基づいて古いファイルを削除または圧縮します。このスクリプトは、Windows システムのメンテナンスやクリーンアップのために使用されます。
.PARAMETER Directory
処理の対象となるディレクトリを指定します。
.PARAMETER FilePattern
処理の対象となるファイルパターンを指定します。デフォルトは全ファイル("*")です。
.PARAMETER CutoffDays
このパラメータは、削除または圧縮を検討するファイルの古さを日単位で指定します。デフォルト値は 366 日です。
.PARAMETER CutoffPercent
ディスクの空き容量がこのパーセンテージ未満の場合に、ファイルの削除処理を開始します。
.PARAMETER Compress
このオプションが指定された場合、対象となるファイルは圧縮されます。
.PARAMETER Logging
...
このコマンドは、30 日以上古い、「Archive-*-*-*-*-*-*-*-*.evtx」という形式の名前のファイルを C:\Temp ディレクトリから圧縮します。
.NOTES
このスクリプトは、管理者権限で実行する必要があります。また、実行する前に影響を受ける可能性のあるファイルを確認してください。
.LINK
